...
var StripeAPI = class {
  ...
  async run(method, arg) {
    const tool24 = this.tools.find((t) => t.method === method);
    if (tool24) {
      const output = JSON.stringify(
        await tool24.execute(this.stripe, this.context, arg)
      );
      return output;
    } else {
...
var isToolAllowed = (tool24, configuration) => {
  return Object.keys(tool24.actions).every((resource) => {
    const permissions = tool24.actions[resource];
...
function StripeTool(stripeAPI, method, description, schema) {
...
var StripeAgentToolkit = class {
  ...
  constructor({
    ...
    const filteredTools = tools_default(context).filter(
      (tool24) => isToolAllowed(tool24, configuration)
    );
    filteredTools.forEach((tool24) => {
      this.tools[tool24.method] = StripeTool(
        this._stripe,
        tool24.method,
        tool24.description,
        tool24.parameters
