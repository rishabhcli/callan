...
__export(index_exports, {
  ...
  registerPaidTool: () => registerPaidTool
});
...
`;
var listCustomersParameters = (_context = {}) => import_zod2.z.object({
  limit: import_zod2.z.number().int().min(1).max(100).optional().describe(
    "A limit on the number of objects to be returned. Limit can range between 1 and 100."
  ),
  email: import_zod2.z.string().optional().describe(
...
var createProductParameters = (_context = {}) => import_zod3.z.object({
  name: import_zod3.z.string().describe("The name of the product."),
  description: import_zod3.z.string().optional().describe("The description of the product.")
});
...
var listProductsParameters = (_context = {}) => import_zod4.z.object({
  limit: import_zod4.z.number().int().min(1).max(100).optional().describe(
...
var createPriceParameters = (_context = {}) => import_zod5.z.object({
  product: import_zod5.z.string().describe("The ID of the product to create the price for."),
  unit_amount: import_zod5.z.number().int().describe("The unit amount of the price in cents."),
  currency: import_zod5.z.string().describe("The currency of the price.")
});
...
var listPricesParameters = (_context = {}) => import_zod6.z.object({
  product: import_zod6.z.string().optional().describe("The ID of the product to list prices for."),
  limit: import_zod6.z.number().int().min(1).max(100).optional().describe(
    "A limit on the number of objects to be returned. Limit can range between 1 and 100, and the default is 10."
  )
});
...
var createPaymentLinkParameters = (_context = {}) => import_zod7.z.object({
  price: import_zod7.z.string().describe("The ID of the price to create the payment link for."),
  quantity: import_zod7.z.number().int().describe("The quantity of the product to include."),
  redirect_url: import_zod7.z.string().optional().describe("The URL to redirect to after the payment is completed.")
});
var tool7 = (context) => ({
...
var createInvoiceParameters = (context = {}) => {
  const schema = import_zod8.z.object({
    customer: import_zod8.z.string().describe("The ID of the customer to create the invoice for."),
    days_until_due: import_zod8.z.number().int().optional().describe("The number of days until the invoice is due.")
  });
  if (context.customer) {
...
var listInvoicesParameters = (context = {}) => {
  const schema = import_zod9.z.object({
    customer: import_zod9.z.string().optional().describe("The ID of the customer to list invoices for."),
...
var createInvoiceItemParameters = (context = {}) => {
  const schema = import_zod10.z.object({
    customer: import_zod10.z.string().describe("The ID of the customer to create the invoice item for."),
    price: import_zod10.z.string().describe("The ID of the price for the item."),
    invoice: import_zod10.z.string().describe("The ID of the invoice to create the item for.")
  });
  if (context.customer) {
    return schema.omit({ customer: true });
  } else {
...
var finalizeInvoiceParameters = (_context = {}) => import_zod11.z.object({
  invoice: import_zod11.z.string().describe("The ID of the invoice to finalize.")
});
...
`;
var retrieveBalanceParameters = (_context = {}) => import_zod12.z.object({});
...
`;
var listCouponsParameters = (_context = {}) => import_zod13.z.object({
  limit: import_zod13.z.number().int().min(1).max(100).optional().describe(
    "A limit on the number of objects to be returned. Limit can range between 1 and 100."
  )
});
...
`;
var createCouponParameters = (_context = {}) => import_zod14.z.object({
  name: import_zod14.z.string().describe(
    "Name of the coupon displayed to customers on invoices or receipts"
  ),
  percent_off: import_zod14.z.number().min(0).max(100).optional().describe(
    "A positive float larger than 0, and smaller or equal to 100, that represents the discount the coupon will apply (required if amount_off is not passed)"
  ),
  amount_off: import_zod14.z.number().describe(
    "A positive integer representing the amount to subtract from an invoice total (required if percent_off is not passed)"
  ),
  currency: import_zod14.z.string().optional().default("USD").describe(
  ...
  duration: import_zod14.z.enum(["once", "repeating", "forever"]).default("once").optional().describe('How long the discount will last. Defaults to "once"'),
  duration_in_months: import_zod14.z.number().optional().describe(
...
var createRefundParameters = (_context = {}) => import_zod15.z.object({
  payment_intent: import_zod15.z.string().describe("The ID of the PaymentIntent to refund."),
  amount: import_zod15.z.number().int().optional().describe("The amount to refund in cents.")
});
...
var listPaymentIntentsParameters = (context = {}) => {
  const schema = import_zod16.z.object({
    customer: import_zod16.z.string().optional().describe("The ID of the customer to list payment intents for."),
    limit: import_zod16.z.number().int().min(1).max(100).optional().describe(
      "A limit on the number of objects to be returned. Limit can range between 1 and 100."
    )
  });
  if (context.customer) {
...
var listSubscriptionsParameters = (context = {}) => {
  const schema = import_zod17.z.object({
    customer: import_zod17.z.string().optional().describe("The ID of the customer to list subscriptions for."),
    price: import_zod17.z.string().optional().describe("The ID of the price to list subscriptions for."),
    status: import_zod17.z.enum([
      "active",
      "past_due",
      "unpaid",
      "canceled",
    ...
    limit: import_zod17.z.number().int().min(1).max(100).optional().describe(
      "A limit on the number of objects to be returned. Limit can range between 1 and 100."
    )
  });
  if (context.customer) {
...
var cancelSubscriptionParameters = (_context = {}) => {
  return import_zod18.z.object({
    subscription: import_zod18.z.string().describe("The ID of the subscription to cancel.")
  });
};
...
var updateSubscriptionParameters = (_context = {}) => {
  return import_zod19.z.object({
    subscription: import_zod19.z.string().describe("The ID of the subscription to update."),
    proration_behavior: import_zod19.z.enum(["create_prorations", "none", "always_invoice", "none_implicit"]).optional().describe(
      "Determines how to handle prorations when the subscription items change."
    ),
    items: import_zod19.z.array(
      import_zod19.z.object({
        id: import_zod19.z.string().optional().describe("The ID of the subscription item to modify."),
        price: import_zod19.z.string().optional().describe("The ID of the price to switch to."),
        quantity: import_zod19.z.number().int().min(1).optional().describe("The quantity of the plan to subscribe to."),
        deleted: import_zod19.z.boolean().optional().describe("Whether to delete this item.")
      })
...
`;
var searchDocumentationParameters = (_context = {}) => import_zod20.z.object({
  question: import_zod20.z.string().describe(
    "The user question about integrating with Stripe will be used to search the documentation."
  ),
  language: import_zod20.z.enum(["dotnet", "go", "java", "node", "php", "ruby", "python", "curl"]).optional().describe(
...
`;
var listDisputesParameters = (_context = {}) => import_zod21.z.object({
  charge: import_zod21.z.string().optional().describe(
    "Only return disputes associated to the charge specified by this charge ID."
  ),
  payment_intent: import_zod21.z.string().optional().describe(
    "Only return disputes associated to the PaymentIntent specified by this PaymentIntent ID."
  ),
  limit: import_zod21.z.number().int().min(1).max(100).default(10).optional().describe(
    "A limit on the number of objects to be returned. Limit can range between 1 and 100, and the default is 10."
  )
});
...
`;
var updateDisputeParameters = (_context = {}) => import_zod22.z.object({
  dispute: import_zod22.z.string().describe("The ID of the dispute to update"),
  evidence: import_zod22.z.object({
    cancellation_policy_disclosure: import_zod22.z.string().max(2e4).optional().describe(
      "An explanation of how and when the customer was shown your refund policy prior to purchase."
    ),
    duplicate_charge_explanation: import_zod22.z.string().max(2e4).optional().describe(
      "An explanation of the difference between the disputed charge versus the prior charge that appears to be a duplicate."
    ),
    uncategorized_text: import_zod22.z.string().max(2e4).optional().describe("Any additional evidence or statements.")
  }).optional().describe(
    "Evidence to upload, to respond to a dispute. Updating any field in the hash will submit all fields in the hash for review."
  ),
  submit: import_zod22.z.boolean().optional().describe(
...
// src/modelcontextprotocol/toolkit.ts
var StripeAgentToolkit = class extends import_mcp.McpServer {
  _stripe;
  constructor({
    secretKey,
    configuration
...
async function registerPaidTool(mcpServer, toolName, toolDescription, paramsSchema, paidCallback, options) {
  ...
  const stripe = new import_stripe2.default(options.stripeSecretKey, {
    appInfo: {
